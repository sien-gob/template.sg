...
import { NotifyErrorByEmailService } from 'src/modules/messenger/application/services';
...
export function HandleErrors(props: { code?: string; config?: { logPersistence?: boolean; notifyErrorEmail?: boolean } } = {}) {
  ...
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    if (notifyErrorEmail) {
      const injectNotifyErrorByEmailService = Inject(NotifyErrorByEmailService);
    ...
    descriptor.value = async function (...args: any[]) {
      try {
        ...
        if (notifyErrorEmail && this.notifyByEmailService) {
          const notifyService: NotifyErrorByEmailService = this.notifyByEmailService;
          await notifyService.send(code, message, this.constructor.name);
        }
